...
async function exportPdf(req, res) {
  try {
    ...
    if (htmlPdf) {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="${encodeURIComponent(payload.title)}.pdf"`);
...
async function exportPptx(req, res) {
  try {
    ...
    const blocks = markdownToBlocks(removeCodeBlocks(content), title);
    let slideNumber = 1;
...
module.exports = { exportPdf, exportCodeZip, exportPptx };
